...
class TextDataset(data.Dataset):
    ...
    def prepair_training_pairs(self, index):
        ...
        class_type = self.class_id[index]
        for i in range(len(self.class_id)):
            if self.class_id[i] == class_type:
                if i != index:
                    wrong_ix = i
                    break
        ...
        wrong_label = self.class_id[wrong_ix]
        real_label = class_type
        if self.bbox is not None:
        ...
        return imgs, wrong_imgs, embedding, key, wrong_label-1, real_label-1  # captions
